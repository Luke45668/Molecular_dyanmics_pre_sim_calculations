...
timescale_parameter_check= np.sqrt((SRD_mass_scale_parameter*(lengthscale_parameter**2)/energy_parameter))
scaled_rho_s = rho_s*(lengthscale_parameter**3)/SRD_mass_scale_parameter
scaled_gamma_dot = Gamma_dot * timescale_parameter 
scaled_dynamic_viscosity = eta_s * (lengthscale_parameter**(3))/ (energy_parameter* timescale_parameter)
scaled_nu_s =scaled_dynamic_viscosity/scaled_rho_s #nu_s * timescale_parameter /(lengthscale_parameter**2)
...
volume_solvent_without_solid = new_simbox_side_length**(3)-(2*volume_solid_particle)
...
#%% calculating SRD timestep 

srd1nd=(scaled_nu_s*(energy_parameter*timescale_parameter))/SRD_mass_scale_parameter
srd2nd=(scaled_nu_s*energy_parameter*timescale_parameter/(SRD_mass_scale_parameter))**2
srd6nd=(scaled_temp*gamma_1*gamma_2*(energy_parameter)) * (lengthscale_parameter**2)
srd3nd= 18*scaled_rho_s*SRD_box_size_wrt_solid_beads*SRD_mass_scale_parameter 
srd4nd= scaled_temp*gamma_2*energy_parameter*np.sqrt(SRD_mass_scale_parameter)* lengthscale_parameter
srd5nd= 2*scaled_rho_s*(SRD_box_size_wrt_solid_beads**3)*SRD_mass_scale_parameter*np.sqrt(energy_parameter)


SRD_timestep_cp_1_based_on_sphere_pf_neg_nd = (srd1nd - np.sqrt(srd2nd-(srd6nd/srd3nd)))/(srd4nd/srd5nd)
SRD_timestep_cp_1_based_on_sphere_pf_pos_nd=(srd1nd + np.sqrt(srd2nd-(srd6nd/srd3nd)))/(srd4nd/srd5nd)
#print(SRD_timestep_cp_1_based_on_sphere_pf_neg_nd)

#%% testing the dimensional calculation against the N_d calc
srd1 =nu_s - np.sqrt((nu_s**2) -  (k_b*T_K*gamma_1*gamma_2)/(18*rho_s*SRD_box_size_wrt_solid_beads_check))
srd2=(k_b*T_K*gamma_2) /(2*rho_s*(SRD_box_size_wrt_solid_beads_check**3))
SRD_timestep_cp_1_based_on_sphere_neg_dimensional_check = (srd1/srd2 ) / timescale_parameter
...
for z in range(0,Solvent_bead_SRD_box_density_cp_1.size):
   for i in range(0,box_size_vec.size):
     check[z,i] = m.isclose(SRD_timestep_cp_1_based_on_sphere_pf_neg_nd[z,i],SRD_timestep_cp_1_based_on_sphere_neg_dimensional_check[z,i], abs_tol=0.01)
     if  check[z,i].all()==1:
       print("Non-dimensionalisation correct")
     else:
       print("incorrect non-dimensionalisation")
...
for z in range(0,Solvent_bead_SRD_box_density_cp_1.size):
    for i in range(0,SRD_box_size_wrt_solid_beads.size):
        ...
        if mean_free_path_pf_SRD_particles_cp_mthd_1_neg[z,i]< 0.01*SRD_box_size_wrt_solid_beads[0,i]:
                ...
                Number_MD_steps_per_SRD_with_pf_cp_mthd_1_neg[z,i] = float("NAN")
                Number_MD_steps_per_SRD_with_pf_cp_mthd_1_pos[z,i] = float("NAN")
                number_SRD_particles_wrt_pf_cp_mthd_1[z,i]=float("NAN")

        if number_SRD_particles_wrt_pf_cp_mthd_1[z,i] <100000:
            print()    
        else: 
            Number_MD_steps_per_SRD_with_pf_cp_mthd_1_neg[z,i] = float("NAN")
            Number_MD_steps_per_SRD_with_pf_cp_mthd_1_pos[z,i] = float("NAN")
            print("All tests with SRD count > 100000 removed ")
            mean_free_path_pf_SRD_particles_cp_mthd_1_neg[z,i]= float("NAN")
            mean_free_path_pf_SRD_particles_cp_mthd_1_pos[z,i]= float("NAN")
        if number_SRD_particles_wrt_pf_cp_mthd_1[z,i] >10000:
            print()    
        else: 
            Number_MD_steps_per_SRD_with_pf_cp_mthd_1_neg[z,i] = float("NAN")
            Number_MD_steps_per_SRD_with_pf_cp_mthd_1_pos[z,i] = float("NAN")
            print("All tests with SRD count < 10000 removed ")
            mean_free_path_pf_SRD_particles_cp_mthd_1_neg[z,i]= float("NAN")
            mean_free_path_pf_SRD_particles_cp_mthd_1_pos[z,i]= float("NAN")
        # need to add a constraint on the number of SRd particles
...
realisation_index_ =np.linspace(1, l,l) # [1,2,3]
...
swap_rate=np.array([[3,15,60,300,1200]])
#swap_rate = np.array([3,6,10,15,60,300,1200]) # values chosen from original mp paper
...
thermo_freq = 10000
VP_data_upper=np.zeros((locations_of_non_nan_neg.shape[0],swap_rate.size,9,int(no_timesteps/thermo_freq),number_of_realisations))
VP_data_lower=np.zeros((locations_of_non_nan_neg.shape[0],swap_rate.size,9,int(no_timesteps/thermo_freq),number_of_realisations))
#%% simulation run code for neg solns
for z in range(0,1):#locations_of_non_nan_neg.shape[0]): 
   # for i in range (0,7): 
       # i=1
        for j in range(0,1):#number_of_realisations): #or now just use one realisation 
            for m in range(0,1):#swap_rate.size):#range(0,1):       #range(0,swap_rate.size):
                for k in range(0,1):#swap_number.size):
                    
                    
                    no_SRD=str(int(number_SRD_particles_wrt_pf_cp_mthd_1[locations_of_non_nan_neg[z,0],locations_of_non_nan_neg[z,1]])) # select one input np.ceil(1.1*
                ...
                #    SRD_MD_ratio=str(int(np.ceil( Number_MD_steps_per_SRD_with_pf_cp_mthd_1_neg[z,i])))
                    ...
                    #mass_solid = str(mass_solid_particle)
                    #t_damp = str(t_damp_SRD[j])
                    d_part_sim = str(2*r_particle_scaled)
            ...
            # Run code 
                    ...
                    os.chdir("/Users/lukedebono/documents/LAMMPS_projects_mac_book/OneDrive_1_24-02-2023/solid_sphere_sims")
                    ...
                    os.system('mpirun -np 4 /Users/lukedebono/documents/LAMMPS_projects_mac_book/lammps-23Jun2022/src/lmp_mpi -var d_particle '+d_part_sim+' -var swap_rate '+str(swap_rate[0,m])+' -var swap_number '+str(swap_number[k])+' -var VP_ave_freq '+str(VP_ave_freq)+' -var equilibration_timesteps '+str(equilibration_timesteps)+' -var chunk '+str(chunk)+' -var grid_size '+grid_size+' -var realisation_index '+str(realisation_index_[j])+' -var r_part_sim '+d_part_sim+' -var temp_ '+temp_+' -var lambda '+str(lamda[locations_of_non_nan_neg[z,0],locations_of_non_nan_neg[z,1]])+' -var rand_int '+rand_int+' -var rand_int_1 '+rand_int_1+' -var rand_int_2 '+rand_int_2+' -var no_SRD '+no_SRD+' -var mass_SRD '+mass_SRD+' -var box_size '+box_size+' -var timestep_input '+timestep_input+' -var SRD_MD_ratio '+SRD_MD_ratio+' -var shear_rate ' +shear_rate+' -var dump_freq '+dump_freq+' -var thermo_freq '+thermo_freq+' -var no_timesteps '+no_timesteps+' -in /Users/lukedebono/documents/LAMMPS_projects_mac_book/OneDrive_1_24-02-2023/LSC/M_P_solid_sphere_codes/no_wall_solid_inc_SRD_sim_var_inputs_td_var_no_tstat_no_rescale_no_dump.file')
             ...
             #   os.system('mpirun -np 8 ./lmp_mpi -var equilibration_timesteps '+str(equilibration_timesteps)+' -var chunk '+str(chunk)+' -var grid_size '+grid_size+' -var realisation_index '+str(realisation_index_[j])+' -var r_part_sim '+r_part_sim+' -var temp_ '+temp_+' -var lambda '+str(lamda[j,i])+' -var rand_int '+rand_int+' -var rand_int_1 '+rand_int_1+' -var rand_int_2 '+rand_int_2+' -var no_SRD '+no_SRD+' -var mass_SRD '+mass_SRD+' -var box_size '+box_size+' -var timestep_input '+timestep_input+' -var SRD_MD_ratio '+SRD_MD_ratio+' -var shear_rate ' +shear_rate+' -var dump_freq '+dump_freq+' -var thermo_freq '+thermo_freq+' -var no_timesteps '+no_timesteps+' -in no_wall_pure_SRD_sim_var_inputs_td_var_fix_deform_version.file')
                   # Path_2_VP="/Volumes/Backup Plus/PhD_/Rouse Model simulations/Using LAMMPS imac/"+simulation_file
                    Path_2_VP="/Users/lukedebono/documents/LAMMPS_projects_mac_book/OneDrive_1_24-02-2023/solid_sphere_sims"
                    VP_output_col_count = 4 
                    realisation_name="vel.profile_solid_inc_no_tstat__no_rescale_"+str(rand_int)+"_"+str(realisation_index_[j])+"_"+str(no_SRD)+"_"+str(box_size)+"_"+str(timestep_input)+"_"+str(SRD_MD_ratio)+"_"+str(dump_freq)+"_"+str(thermo_freq)+"_"+str(no_timesteps)+"_T_"+str(temp_)+"_lbda_"+str(lamda[locations_of_non_nan_neg[z,0],locations_of_non_nan_neg[z,1]])+"_SR_"+str(swap_rate[0,m])+"_SN_"+str(swap_number[k])  
                    #realisation_name="vel.profile_solid_inc_no_tstat__no_rescale_203267_1.0_33315_16.0_0.01_5_1000_10000_200000_T_5e-05_lbda_0.0025850369836944932_SR_1200_SN_1"
