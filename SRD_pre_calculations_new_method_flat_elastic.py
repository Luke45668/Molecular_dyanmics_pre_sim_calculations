...
#abs_path_2_lammps_exec='/home/ucahlrl/simulation_run_folder/lammps-23Jun2022_with_SRD_pol/build_MYRIAD/lmp_mpi'
...
internal_stiffness=np.array([20,40,60,80,100])
...
def compute_timesteps_for_strain(total_strain,erate,md_timestep):
      no_timestep_=(np.round((total_strain/(erate*md_timestep)),decimals=-3)).astype('int')

      return no_timestep_

# for MYRIAD run
total_strain=20
no_timestep_=compute_timesteps_for_strain(total_strain,erate,md_timestep)
# for test run 
#no_timestep_=np.array([5000,4000,3000,2000,1000,600])

var_choice_1=erate
var_choice_2=internal_stiffness

def sim_file_prod_flat_elastic_individual_MYRIAD(coordinates_tuple_3d,erate,equilibrium_triangle_side_length,var_choice_1,var_choice_2,internal_stiffness,data_transfer_instructions,extra_code,wd_path,np_req,num_task_req,tempdir_req,wall_time,ram_requirement,realisation_index_,VP_ave_freq,abs_path_2_lammps_exec,abs_path_2_lammps_script,no_timestep_,thermo_freq,md_timestep,i_,j_,box_size_bar,box_size_index,Path_2_shell_scirpts,Path_2_generic,fluid_name):
    ...
    #for k in range(0,1):   
            ...
            #for m in range(0,1):  
                    for j in range(i_,j_):
                        param_set_code=str(np.random.randint(0, 1000000))
                        simulation_run_name=fluid_name+'_'+str(sim_batchcode)+'_'+param_set_code+'_realisation_'+str(j)+'_Bk_'+str(bending_stiffness[0])+'_timestep_'+str(md_timestep)+'_no_timesteps_'+str(no_timestep_[k])+'_intstiff_'+str(var_choice_2[m])+'_eqsl_'+str(equilibrium_triangle_side_length)+'_erate_'+str(erate[k])+'_'
                        run_code=''
                        no_SRD=str(int(srd_count[box_size_index])) 
                        #print(no_SRD)
                        box_size = str(box_size_bar[box_size_index])
                        ...
                        run_code_individual ='mpirun -np '+str(num_proc)+'  '+abs_path_2_lammps_exec+' -var erate_in '+str(erate[k])+' -var equilirbium_triangle_side_length '+str(equilibrium_triangle_side_length)+\
...
# %%
run_code_list=sim_file_prod_flat_elastic_individual_MYRIAD(coordinates_tuple_3d,erate,equilibrium_triangle_side_length,var_choice_1,var_choice_2,internal_stiffness,data_transfer_instructions,extra_code,wd_path,np_req,num_task_req,tempdir_req,wall_time,ram_requirement,realisation_index_,VP_ave_freq,abs_path_2_lammps_exec,abs_path_2_lammps_script,no_timestep_,thermo_freq,md_timestep,i_,j_,box_size_bar,box_size_index,Path_2_shell_scirpts,Path_2_generic,fluid_name)
...
os.chdir("/Volumes/Backup Plus 1/PhD_/Rouse Model simulations/Using LAMMPS imac/Simulation_run_folder/test_run_flat_elastic_100_realisations/")
MyFile=open('dist_test.sh','w')
